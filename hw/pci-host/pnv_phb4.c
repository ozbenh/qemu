...
static void pnv_phb4_update_xsrc(PnvPHB4 *phb)
{
    ...
    for (i = 0; i < xsrc->nr_irqs; i++) {
        ...
        xive_source_irq_set(xsrc, i, lsi);
    }
}
...
static void pnv_phb4_set_irq(void *opaque, int irq_num, int level)
{
    ...
    id_base = GETFIELD(PHB_LSI_SOURCE_ID, phb->regs[PHB_LSI_SOURCE_ID >> 3]);
    id_base <<= 3;
    qemu_set_irq(xive_source_qirq(&phb->xsrc, id_base + irq_num), level);
}
                               ...
                               uint64_t data, unsigned size)
{
    ...
    qemu_irq_pulse(xive_source_qirq(&phb->xsrc, src));
}
...
static void pnv_phb4_xive_notify(XiveFabric *xf, uint32_t srcno)
{
