...
void pnv_phb3_remap_irqs(PnvPHB3 *phb)
{
    ...
    if ((global + PNV_PHB3_NUM_LSI) > count) {
        phb3_error("LSIs out of reach: LSI base=%d total irq=%d", global,
                   count);
    }
    ...
    if ((comp & mask) != comp) {
        phb3_error("IRQ compare bits not in mask: comp=0x%x mask=0x%x",
                   comp, mask);
        comp &= mask;
    }
    /* Setup LSI offset */
...
void pnv_phb3_reg_write(void *opaque, hwaddr off, uint64_t val, unsigned size)
{
    ...
    if (size != 8 || off & 0x7) {
        phb3_error("Invalid register access, offset: 0x%"PRIx64" size: %d",
                   off, size);
        return;
    }
    ...
    if (tps == 0) {
        ...
        while ((lev--) >= 0) {
            ...
            if ((lev >= 0) && !(tce & 3)) {
                ...
                phb3_error(" xlate %"PRIx64":%c TVE=%"PRIx64, addr,
                           is_write ? 'W' : 'R', tve);
                phb3_error(" tta=%"PRIx64" lev=%d tts=%d tps=%d",
                           tta, lev, tts, tps);
                return;
            }
            sh -= tbl_shift;
...
static void pnv_phb3_instance_init(Object *obj)
{
    ...
    /* Root Port */
    object_initialize(&phb->root, sizeof(phb->root), TYPE_PNV_PHB3_ROOT_PORT);
...
static void pnv_phb3_realize(DeviceState *dev, Error **errp)
{
    ...
    /* LSI sources */
    object_property_add_const_link(OBJECT(&phb->lsis), "xics", OBJECT(pnv),
                                   &error_abort);
    ...
    /* MSI sources */
    object_property_add_const_link(OBJECT(&phb->msis), "xics", OBJECT(pnv),
                                   &error_abort);
