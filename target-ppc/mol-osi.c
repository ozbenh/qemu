...
static target_ulong ppc_mol_osi_log_putc(PowerPCCPU *cpu, target_ulong *args)
{
    uint8_t c = args[0];

    qemu_chr_fe_write(serial_hds[0], &c, 1);

    return 0;
}

typedef target_ulong (*ppc_mol_osi_call_t)(PowerPCCPU *cpu, target_ulong *args);

static ppc_mol_osi_call_t       ppc_mol_osi_calls[NUM_OSI_SELECTORS] = {
    [OSI_LOG_PUTC] = ppc_mol_osi_log_putc,
};

void ppc_mol_osi_call(PowerPCCPU *cpu)
{
    CPUPPCState *env = &cpu->env;
    target_ulong sel = env->gpr[5];

    if (sel == OSI_CALL_AVAILABLE) {
        env->gpr[3] = env->gpr[6] < NUM_OSI_SELECTORS &&
                ppc_mol_osi_calls[env->gpr[6]] != NULL;
        return;
    }
    if (sel >= NUM_OSI_SELECTORS || ppc_mol_osi_calls[sel] == NULL) {
        qemu_log_mask(LOG_GUEST_ERROR, "Invalid OSI call "
                      TARGET_FMT_lx "\n", sel);
        return;
    }
    env->gpr[3] = ppc_mol_osi_calls[sel](cpu, &env->gpr[6]);
}
